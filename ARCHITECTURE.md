...
`schema-rust` generates `Input`, `Output`, short-header routes,
...
| Each variant's NOTA head matches the contract-local verb declared in `schema/lib.schema`. | Generated by `schema-rust`; round-trip tests assert each variant's head. |
...
| Contract crate dependencies use a named API reference (branch or tag), not a raw revision pin. | `Cargo.toml` review: `nota`, `signal-frame`, and `schema-rust` are declared `git = "..."` with a named-branch shape; raw `rev = "..."` pins are not used. |
...
└── schema/
    ├── mod.rs
    └── lib.rs            — checked-in schema-rust generated wire contract
schema/
