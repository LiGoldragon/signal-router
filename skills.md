...
- Router-owned Signal vocabulary: `Input`, `Output`,
...
- **Every request variant declares a contract-local operation head.**
  `schema/router.ethos` is the source of truth; tests assert the exact
  heads.
...
- **Pin upstream contracts exactly.** Cargo names the published producer
  revision for each upstream contract.

## Editing patterns

### Adding a new observation request

1. Write the canonical Dotos example for the request and the expected
   reply in `examples/canonical.dotos` first. Per
   `~/primary/skills/contract-repo.md` §"Examples-first round-trip
   discipline", the example is the falsifiable spec.
2. Declare the payload struct and root variant in `schema/router.ethos`.
3. Extend `src/binding.rs`, the current Rust binding, in the same change.
4. Add the rkyv round-trip test in `tests/round_trip.rs`.
5. Add the Dotos round-trip witness for the new variant in the
...
2. Declare the semantic stream in the schema surface once the schema
   stream surface is available for this contract, with both a
...
The interface-derived codec emits a root variant's Dotos head as the operation
