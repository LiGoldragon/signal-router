// Current Rust binding of `schema/router.ethos`.
...
impl Input {
    ...
    pub fn short_header(&self) -> u64 {
        match self {
            Self::Summary(_) => 0x0001,
            Self::MessageTrace(_) => 0x0101,
            Self::ChannelState(_) => 0x0201,
            Self::ForwardMessage(_) => 0x0301,
            Self::SubmitRoutedObjects(_) => 0x0401,
            Self::SessionClientHello(_) => 0x0501,
            Self::SessionClientProof(_) => 0x0601,
            Self::SessionData(_) => 0x0701,
            Self::RegisterActor(_) => 0x0801,
        }
    }
    pub fn route_from_short_header(header: u64) -> Result<InputRoute, SignalFrameError> {
        match header {
            0x0001 => Ok(InputRoute::Summary),
            0x0101 => Ok(InputRoute::MessageTrace),
            0x0201 => Ok(InputRoute::ChannelState),
            0x0301 => Ok(InputRoute::ForwardMessage),
            0x0401 => {
            ...
            0x0701 => Ok(InputRoute::SessionData),
            0x0801 => Ok(InputRoute::RegisterActor),
            _ => {
...
pub enum RouterWire {}
#[rustfmt::skip]
impl signal_frame::WireContract for RouterWire {
...
#[rustfmt::skip]
pub type Frame = signal_frame::BoundExchangeFrame<RouterWire, Input, Output>;
#[rustfmt::skip]
...
impl Input {
    pub fn into_frame(self, exchange: signal_frame::ExchangeIdentifier) -> Frame {
        ...
        let route = request.route().expect("Input routes fit the router interface");
...
impl Output {
    pub fn into_reply_frame(self, exchange: signal_frame::ExchangeIdentifier) -> Frame {
        ...
        Frame::new(signal_frame::WireRoute::new(
