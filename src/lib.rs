//! Signal contract for the ordinary `router` surface.
//!
...
/// Canonical Ethos declaration of the Router interface. Rust in `binding` is
/// its current behavioral binding, not a second source of interface truth.
pub const ROUTER_INTERFACE_SOURCE: &str = include_str!("../schema/router.ethos");

pub use binding::*;

impl RouterBootstrapOperation {
    #[cfg(feature = "dotos-text")]
    pub fn from_dotos(text: &str) -> Result<Self, DotosDecodeError> {
...
impl RouterBootstrapDocument {
    ...
    #[cfg(feature = "dotos-text")]
    pub fn from_dotos_lines(text: &str) -> Result<Self, DotosDecodeError> {
        let mut operations = Vec::new();
        for line in text.lines().map(str::trim).filter(|line| !line.is_empty()) {
            operations.push(RouterBootstrapOperation::from_dotos(line)?);
        }
        Ok(Self::from_operations(operations))
    }

    #[cfg(feature = "dotos-text")]
    pub fn to_dotos_lines(&self) -> String {
        let mut text = String::new();
        for operation in self.operations() {
            text.push_str(operation.to_dotos().as_str());
            text.push('\n');
        }
        text
    }
...
//
// Ergonomic public constructors/accessors over the interface session
// vocabulary. The binding's octet wrappers (`SealedOctets`, `KeyConfirmation`)
