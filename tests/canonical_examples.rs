...
fn canonical_request_examples_round_trip() {
    ...
    for (value, canonical_text) in expected {
        ...
        let decoded = DotosSource::new(&text).parse::<Input>().expect("decode");
        assert_eq!(decoded, value, "decode for {text}");

        assert!(CANONICAL.contains(&text), "missing canonical Dotos: {text}");
...
fn canonical_reply_examples_round_trip() {
    ...
    for (value, canonical_text) in expected {
        let text = value.to_dotos();
        let decoded = DotosSource::new(&text).parse::<Output>().expect("decode");
        assert_eq!(decoded, value, "decode for {text}");
...
fn canonical_register_remote_router_bootstrap_example_round_trips() {
    ...
    let decoded = DotosSource::new(&text)
        ...
        .expect("decode");
    assert_eq!(decoded, operation, "decode for {text}");
...
fn canonical_extended_daemon_configuration_example_round_trips() {
    ...
    let canonical_text = "(/run/persona/X/router.sock 384 /run/persona/X/router-meta.sock 384 /run/persona/X/router-supervision.sock 384 /var/lib/persona/X/router.sema (Some /var/lib/persona/X/router-bootstrap.dotos) (UnixUser 1000) (Some [|[200:1234::1]:9930|]) ouranos-router (Some /run/persona/X/criome.sock))";

    let text = configuration.to_dotos();
    let decoded = DotosSource::new(&text)
        .parse::<RouterDaemonConfiguration>()
        .expect("decode");
    assert_eq!(decoded, configuration, "decode for {text}");
