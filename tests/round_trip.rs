...
use signal_frame::{
    ExchangeIdentifier, ExchangeLane, LaneSequence, Reply, SessionEpoch, SignalOperationHeads,
    SubReply,
};
use signal_router::{
...
fn round_trip_request(request: Input) {
    let frame = request.clone().into_frame(exchange());
...
#[cfg(feature = "dotos-text")]
#[test]
fn router_forward_request_round_trips_through_dotos_text() {
    round_trip_dotos(
...
#[cfg(feature = "dotos-text")]
#[test]
fn router_forward_accepted_reply_round_trips_through_dotos_text() {
    round_trip_dotos(Output::forward_accepted(7.into()), "ForwardAccepted.7");
}
...
#[cfg(feature = "dotos-text")]
#[test]
fn router_daemon_configuration_round_trips_through_dotos_text() {
    let configuration = RouterDaemonConfiguration::from(RouterDaemonConfigurationParts {
        ...
        bootstrap_path: Some(String::from("/var/lib/persona/X/router-bootstrap.dotos").into()),
...
#[cfg(feature = "dotos-text")]
#[test]
fn bootstrap_register_actor_operation_round_trips_through_dotos_line() {
    ...
    let text = operation.to_dotos();
    assert_eq!(
        text,
        "RegisterActor.{{responder 42 Some.{HarnessSocket /tmp/responder.harness.sock None}} None}"
    );
    assert_eq!(
        RouterBootstrapOperation::from_dotos(&text).expect("decode bootstrap operation"),
        operation
    );
}
...
#[cfg(feature = "dotos-text")]
#[test]
fn bootstrap_register_actor_with_remote_home_round_trips_through_dotos_line() {
...
#[cfg(feature = "dotos-text")]
#[test]
fn bootstrap_direct_message_grant_operation_round_trips_through_dotos_line() {
...
#[cfg(feature = "dotos-text")]
#[test]
fn bootstrap_register_remote_router_operation_round_trips_through_dotos_line() {
...
fn bootstrap_document_owns_line_vocabulary_for_manager_and_router() {
    ...
    let text = document.to_dotos_lines();
    let recovered =
        RouterBootstrapDocument::from_dotos_lines(&text).expect("decode bootstrap document");
...
#[cfg(feature = "dotos-text")]
#[test]
fn router_observation_operation_kind_round_trips_through_dotos_text() {
    round_trip_dotos(RouterObservationScope::MessageTrace, "MessageTrace");
}
